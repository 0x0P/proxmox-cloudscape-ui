...
export async function POST(request: NextRequest) {
  ...
  if (!node) {
    return Response.json({ error: "Missing node" }, { status: 400 });
  }
  ...
  const vncRes = await fetch(
    `${PROXMOX_HOST}/api2/json/nodes/${node}/vncshell`,
    {
      method: "POST",
      headers,
      body: "websocket=1",
  ...
  if (!vncRes.ok) {
    ...
    return Response.json({ error: `vncshell failed: ${text}` }, { status: vncRes.status });
  }
